...
from netket.nn import activation
...
class myRBM(nk.nn.Module):
    ...
    def single_evaluation(self, x):
        ...
        W = self.param("weight0", nk.nn.initializers.normal(0.01), (int(x.shape[-1]*self.alpha), x.shape[-1]), complex)

        x2 = W@x + h_bias
        #x3 = nk.nn.activation.logcosh(x2)
        x3 = jnp.log(jnp.cosh(x2))
...
class mySymmRBM(nk.nn.Module):
    ...
    def single_evaluation(self, x):
        ...
        W = self.param("weight0", nk.nn.initializers.normal(0.5), (self.alpha, self.filter_len, ), complex)
        ...
        #features = jnp.log(jnp.cosh(features))
        features = jnp.log(1+jnp.exp(features)) # softsign
...
class my2LSymmRBM(nk.nn.Module):
    ...
    def single_evaluation(self, x):
        ...
        if self.filter_len == None:
            self.filter_len = x.shape[-1]
        W = self.param("weight0", nk.nn.initializers.normal(0.5), (self.alpha, self.filter_len, ), complex)
        W1 = self.param("weight1", nk.nn.initializers.normal(0.01), (self.alpha, ), complex)
        b2 = self.param("b2", nk.nn.initializers.normal(0.01), (1,), complex)
        ...
        features = jax.vmap(localFeatures)(jnp.arange(x.shape[-1]))
        features = nk.nn.activation.logcosh(features)
        features2 = jnp.sum(features, axis=0)
        y = jnp.dot(W1, features2) + b2[0]
        #y = nk.nn.activation.logcosh(y)
